"""
v22b — Conditional modeling with attention-weighted patient context (raw RR).

Like v22a, we condition query prediction on patient context built from RR support segments
from the SAME patient episode.

Difference vs v22a:
  - v22a context = mean(h_support)
  - v22b context is query-specific:
      weights(q,s) = softmax(cos(h_query, h_support) / temperature)
      context(q) = sum_s weights(q,s) * h_support(s)
  - RR preprocessing: raw RR (no per-patient baseline removal)

Episode sampling:
  - support_k segments and query_k segments are sampled from each patient episode.
  - Training uses SmoothL1Loss on predicted TTE (minutes).
Validation:
  - patient-level disjoint split (train patients disjoint from val patients)
  - For each val patient: use first support_k segments as support; predict for remaining segments.
"""
...
class TTEPredictorConditionalAttention(nn.Module):
    """
    Encoder -> GRU -> h embeddings per segment.
    For each query embedding, attend over support embeddings and predict TTE.
    """
    ...
    def forward(self, support_rr: torch.Tensor, query_rr: torch.Tensor) -> torch.Tensor:
        ...
        # Cosine similarity for attention weights (query-specific)
        ...
        # context: [P, Q, D] = sum_s weights * h_support
        context = torch.einsum("pqs,p sd->pqd", weights, h_support)
...
def centroid_diagnostic_h(model, rr_data, rr_pids, train_pid_set, val_pid_set, device):
    """
    Centroid probe on GRU hidden state embeddings h.
    Build centroids from train patients only; evaluate seen accuracy + unseen AUROC.
    """
    ...
    va_emb, va_pid = emb_all[va_mask], pid_all[va_mask]
...
try:
    ...
    config = {
        ...
        "epochs": 120,
        "lr": 1e-3,
        "batch_patients": 8,      # P per episode
        "support_k": 5,
        "query_k": 3,
    ...
    run = wandb.init(
        ...
        name="v22b-conditional-attention-context-raw-rr",
        config=config,
    )
    ...
    # Load SR-only dataset and filter to excellent
    ...
    def sample_episode(split_patient_ids, indices_by_pid):
        ...
        for pid in chosen:
            ...
            if len(idx_pool) >= total_needed:
                ...
                # If too few segments, allow replacement
                s_idx = np.random.choice(
                    idx_pool, size=config["support_k"], replace=True
                )
                q_idx = np.random.choice(
                    idx_pool, size=config["query_k"], replace=True
                )

            support_list.append(all_data[s_idx])
            query_list.append(all_data[q_idx])
            tte_list.append(all_tte_min[q_idx])
            pid_list.append(np.full((config["query_k"],), pid, dtype=np.int64))

        support_rr = torch.stack(support_list, dim=0).to(device)
        query_rr = torch.stack(query_list, dim=0).to(device)
        query_tte = torch.stack(tte_list, dim=0).to(device)  # [P, query_k]
        query_pids = torch.stack([torch.tensor(x, dtype=torch.long) for x in pid_list], dim=0)
    ...
    for epoch in range(config["epochs"]):
        ...
        for _ in range(config["episodes_per_epoch"]):
            support_rr, query_rr, query_tte, _ = sample_episode(train_pids, train_indices_by_pid)
            optimizer.zero_grad()
        ...
        # Validation: deterministic support/query split within each patient
        model.eval()
        v_preds = []
        v_actual = []
        v_pids_all = []
        ...
        if (epoch + 1) % 5 == 0 or epoch == 0:
            logger.info(
                f"[v22b] ep {epoch+1:>3}/{config['epochs']} | train MAE={train_mae:.2f} rho={train_rho:.3f} | "
                f"val MAE={val_mae:.2f} rho={val_rho:.3f} R2={val_r2:.3f} | pp_rho_mean={v_mean_pp_rho:.3f}"
            )

        if patience_ctr >= config["patience"]:
            logger.info(f"[v22b] early stop at epoch {epoch+1} best val MAE={best_val_mae:.2f}")
            break
    ...
    logger.info("[v22b] centroid diagnostic on embeddings h ...")
    ...
    logger.info(
        f"[v22b] DONE | centroid_seen_acc={cd['seen_acc']:.4f} centroid_unseen_auroc={cd['unseen_auroc']:.4f}"
    )

    # Quick plot on last validation predictions is not stored; keep minimal for now.
