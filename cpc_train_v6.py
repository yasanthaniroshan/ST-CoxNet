...
def time_infonce(
    ...
    temperature=0.1,
    tau=0.1,
    use_knn_positives=False,
    knn_k=8,
):
    if z.size(0) != time.size(0):
        raise ValueError(
            f"time_infonce batch mismatch: z batch={z.size(0)} vs time batch={time.size(0)}"
        )
    if z.size(0) < 2:
    ...
    time_diff = torch.abs(time[:, None] - time[None, :])
    weights = torch.exp(-time_diff / tau)

    # Exclude self-pairs from both logits and target weights.
    ...
    # Optional: keep only k nearest-time neighbors as soft positives.
    ...
    # Re-normalize target weights row-wise after masking.
...
try:
    ...
    config={
        ...
        "cpc_lr": 1e-3,
        "cox_lr": 1e-4,
        "time_infonce_tau": 0.05,
        "time_infonce_use_knn_positives": True,
        "time_infonce_knn_k": 4,
    }
    run = wandb.init(
        entity="eml-labs",
        project="CPC-Time-Infonce", 
        config=config,
    )
    ...
    for cpc_epoch in range(cpc_epochs):
        ...
        validation_time_loss = 0
        validation_cpc_loss = 0
        for rr,label,time in train_data_loader:
            ...
            time_loss = time_infonce(
                torch.mean(context, dim=1),
                time,
                temperature=config["temperature"],
                tau=config["time_infonce_tau"],
                use_knn_positives=config["time_infonce_use_knn_positives"],
                knn_k=config["time_infonce_knn_k"],
            )
            total_step_loss = 0.1*time_loss + loss
        ...
        for rr,label,time in validation_data_loader:
            ...
            with torch.no_grad():
                ...
                validation_loss += 0.1*time_loss.item() + loss.item()
                validation_time_loss += time_loss.item()
                validation_cpc_loss += loss.item()
        ...
        run.log({
            ...
            "cpc_train_time_loss": total_time_loss / len(train_data_loader),
            "cpc_train_cpc_loss": total_cpc_loss / len(train_data_loader),
            "cpc_validation_time_loss": validation_time_loss / len(validation_data_loader),
            "cpc_validation_cpc_loss": validation_cpc_loss / len(validation_data_loader),
            "pca": wandb.Image(fig)
        })
        plt.close(fig)
