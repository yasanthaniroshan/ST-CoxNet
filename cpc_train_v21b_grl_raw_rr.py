"""
v21b — GRL adversarial patient-invariance + raw RR (no baseline removal).
...
class TTEPredictorGRL(nn.Module):
    def __init__(self, latent_dim=64, hidden_dim=128, dropout=0.2, n_patients=10):
        super().__init__()
        ...
        self.patient_head = nn.Sequential(
            nn.Linear(hidden_dim, 64), nn.ReLU(), nn.Dropout(dropout), nn.Linear(64, n_patients),
        )
    ...
    def forward(self, rr_windows):
        ...
        _, h = self.gru(z)
        h = h.squeeze(0)
        tte_pred = self.head(h).squeeze(-1)
        pat_logits = self.patient_head(self.grl(h))
        return tte_pred, pat_logits
...
def centroid_diagnostic(model, train_loader, val_loader, train_pid_set, val_pid_set, device):
    ...
    pred_c = np.argmin(d_tr, axis=1); true_c = np.array([pid2c[p] for p in tr_pid])
...
try:
    ...
    config = {
        ...
        "relative_rr": False,
        "lambda_adv_max": 1.0, "lambda_ramp_epochs": 30,
    }

    run = wandb.init(entity="eml-labs", project="CPC-New-Temporal-Ranking",
                     name="v21b-grl-raw-rr", config=config)
    ...
    n_train_patients = len(train_pid_set); n_val_patients = len(val_pid_set)
    train_cls = torch.tensor([pid2cls[int(p)] for p in train_pid_seg.numpy()], dtype=torch.long)

    logger.info(f"Patient split: {n_train_patients} train | {n_val_patients} val")
    logger.info(f"Segments: {len(train_data)} train | {len(val_data)} val")

    train_loader = DataLoader(TensorDataset(train_data, train_tte, train_pid_seg, train_cls),
    ...
    model = TTEPredictorGRL(config["latent_dim"], config["hidden_dim"],
                            config["dropout"], n_train_patients).to(device)
    logger.info(f"Params: {sum(p.numel() for p in model.parameters() if p.requires_grad):,} | {device}")

    optimizer = optim.AdamW(model.parameters(), lr=config["lr"], weight_decay=1e-4)
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=config["epochs"],
                                                      eta_min=config["lr"] * 0.01)
    tte_loss_fn = nn.SmoothL1Loss(); pat_loss_fn = nn.CrossEntropyLoss()

    pbar = tqdm(total=config["epochs"], desc="v21b GRL+rawRR")
    best_val_mae = float("inf"); patience_ctr = 0

    for epoch in range(config["epochs"]):
        lam = config["lambda_adv_max"] * min(1.0, epoch / max(config["lambda_ramp_epochs"], 1))
        model.grl.set_lambda(lam)
        model.train()
        t_loss, t_adv, t_n, t_correct = 0.0, 0.0, 0, 0
        t_preds, t_actual, t_pids_ep = [], [], []

        for rr, tte, pid, cls in train_loader:
            rr, tte, cls = rr.to(device), tte.to(device), cls.to(device)
            optimizer.zero_grad()
            tte_pred, pat_logits = model(rr)
            l_tte = tte_loss_fn(tte_pred, tte); l_pat = pat_loss_fn(pat_logits, cls)
            loss = l_tte + lam * l_pat
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
            bsz = rr.size(0)
            t_loss += l_tte.item()*bsz; t_adv += l_pat.item()*bsz; t_n += bsz
            t_correct += (pat_logits.argmax(1) == cls).sum().item()
            t_preds.append(tte_pred.detach().cpu().numpy())
            t_actual.append(tte.cpu().numpy()); t_pids_ep.append(pid.numpy())

        scheduler.step()
        t_preds = np.concatenate(t_preds); t_actual = np.concatenate(t_actual)
        ...
        pat_acc = t_correct / max(t_n, 1)

        model.eval(); v_preds, v_actual, v_pids_ep = [], [], []
        with torch.no_grad():
            for rr, tte, pid in val_loader:
                tte_pred, _ = model(rr.to(device))
                v_preds.append(tte_pred.cpu().numpy()); v_actual.append(tte.numpy())
                v_pids_ep.append(pid.numpy())
        v_preds = np.concatenate(v_preds); v_actual = np.concatenate(v_actual)
        ...
        if v_mae < best_val_mae: best_val_mae = v_mae; patience_ctr = 0; torch.save(model.state_dict(), "v21b_best.pth")
        else: patience_ctr += 1

        run.log({"epoch": epoch+1, "lambda_adv": lam,
                 "train/tte_loss": t_loss/max(t_n,1), "train/adv_loss": t_adv/max(t_n,1),
                 "train/pat_acc": pat_acc, "train/mae": t_mae, "train/rho": t_rho,
                 "val/mae": v_mae, "val/rho": v_rho, "val/r2": v_r2, "val/pp_rho": v_mean_pp})

        if (epoch+1) % 10 == 0 or epoch == config["epochs"]-1:
            pbar.write(f"Ep {epoch+1:>3} | MAE {t_mae:.2f}/{v_mae:.2f} | ρ {t_rho:.3f}/{v_rho:.3f} | pat_acc {pat_acc:.3f} | λ {lam:.2f}")
        pbar.update(1)
    ...
    model.load_state_dict(torch.load("v21b_best.pth", weights_only=True)); model.eval()
    v_preds, v_actual, v_pids_all = [], [], []
    with torch.no_grad():
        for rr, tte, pid in val_loader:
            p, _ = model(rr.to(device))
            v_preds.append(p.cpu().numpy()); v_actual.append(tte.numpy()); v_pids_all.append(pid.numpy())
    ...
    logger.info(f"\n{'='*65}")
    logger.info(f"FINAL v21b | MAE={g_mae:.2f} | ρ={g_rho:.4f} | R²={g_r2:.4f} | pp_ρ={g_pp:.4f}")
    logger.info(f"{'='*65}")

    train_loader_3 = DataLoader(TensorDataset(train_data, train_tte, train_pid_seg),
                                batch_size=config["batch_size"], shuffle=False)
    val_loader_3 = DataLoader(TensorDataset(val_data, val_tte, val_pid_seg),
                              batch_size=config["batch_size"], shuffle=False)
    cd = centroid_diagnostic(model, train_loader_3, val_loader_3, train_pid_set, val_pid_set, device)
    logger.info(f"Centroid probe: seen_acc={cd['seen_acc']:.4f} | unseen_auroc={cd['unseen_auroc']:.4f}")

    fig = plt.figure(figsize=(20, 6))
    fig.suptitle(f"v21b GRL+rawRR | MAE={g_mae:.2f} | ρ={g_rho:.3f} | pp_ρ={g_pp:.3f} | R²={g_r2:.3f}", fontsize=13)
    ...
    plt.tight_layout(); plt.savefig("v21b_results.png", dpi=150, bbox_inches="tight"); plt.close(fig)
