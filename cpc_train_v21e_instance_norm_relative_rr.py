"""
v21e — InstanceNorm on GRU embedding + relative RR.

Excellent-only patients, patient-level 0.85/0.15 split.
InstanceNorm1d on the GRU hidden state removes per-sample mean/variance,
stripping patient-specific scale from the embedding.
"""
...
class TTEPredictorIN(nn.Module):
    """Encoder -> GRU -> InstanceNorm(h) -> MLP -> TTE."""
    ...
    def __init__(self, latent_dim=64, hidden_dim=128, dropout=0.2):
        ...
        # We will apply it over the GRU sequence output (L=T), not over the
        # single-step hidden vector.
...
def relative_rr_remove_patient_baseline(all_data, all_pids, eps=1e-8):
    ...
    for pid in np.unique(all_pids.cpu().numpy()):
        ...
        x = out[mask]; mu, sd = float(x.mean()), float(x.std(unbiased=False))
...
try:
    ...
    run = wandb.init(entity="eml-labs", project="CPC-New-Temporal-Ranking",
                     name="v21e-instancenorm-relative-rr", config=config)
    ...
    pbar = tqdm(total=config["epochs"], desc="v21e IN+relRR")
    ...
    for epoch in range(config["epochs"]):
        ...
        if v_mae < best_val_mae: best_val_mae = v_mae; patience_ctr = 0; torch.save(model.state_dict(), "v21e_best.pth")
        else: patience_ctr += 1
    ...
    model.load_state_dict(torch.load("v21e_best.pth", weights_only=True)); model.eval()
    ...
    logger.info(f"\n{'='*65}")
    logger.info(f"FINAL v21e | MAE={g_mae:.2f} | ρ={g_rho:.4f} | R²={g_r2:.4f} | pp_ρ={g_pp:.4f}")
    logger.info(f"{'='*65}")
    ...
    fig.suptitle(f"v21e IN+relRR | MAE={g_mae:.2f} | ρ={g_rho:.3f} | pp_ρ={g_pp:.3f} | R²={g_r2:.3f}", fontsize=13)
    ...
    plt.tight_layout(); plt.savefig("v21e_results.png", dpi=150, bbox_inches="tight"); plt.close(fig)
