"""
v21d — MMD patient-invariance regularization + raw RR (no baseline removal).
...
def rbf_mmd2(x, y, bandwidths=(0.1, 1.0, 10.0)):
    xx = torch.cdist(x, x, p=2).pow(2); yy = torch.cdist(y, y, p=2).pow(2); xy = torch.cdist(x, y, p=2).pow(2)
...
def batch_mmd(h, pids, bandwidths=(0.1, 1.0, 10.0)):
    ...
    groups = [h[pids == p] for p in unique if (pids == p).sum() >= 2]
    if len(groups) < 2:
        return torch.tensor(0.0, device=h.device)
    total = torch.tensor(0.0, device=h.device); count = 0
    for i in range(len(groups)):
        for j in range(i + 1, len(groups)):
            total = total + rbf_mmd2(groups[i], groups[j], bandwidths); count += 1
...
try:
    ...
    run = wandb.init(entity="eml-labs", project="CPC-New-Temporal-Ranking",
                     name="v21d-mmd-raw-rr", config=config)
    ...
    for epoch in range(config["epochs"]):
        ...
        for rr, tte, pid in train_loader:
            ...
            l_tte = tte_loss_fn(tte_pred, tte); l_mmd = batch_mmd(h, pid_dev)
            loss = l_tte + lam * l_mmd; loss.backward()
        ...
        if v_mae < best_val_mae: best_val_mae = v_mae; patience_ctr = 0; torch.save(model.state_dict(), "v21d_best.pth")
        else: patience_ctr += 1
    ...
    model.load_state_dict(torch.load("v21d_best.pth", weights_only=True)); model.eval()
    ...
    logger.info(f"\n{'='*65}")
    logger.info(f"FINAL v21d | MAE={g_mae:.2f} | ρ={g_rho:.4f} | R²={g_r2:.4f} | pp_ρ={g_pp:.4f}")
    logger.info(f"{'='*65}")
    ...
    fig.suptitle(f"v21d MMD+rawRR | MAE={g_mae:.2f} | ρ={g_rho:.3f} | pp_ρ={g_pp:.3f} | R²={g_r2:.3f}", fontsize=13)
    ...
    plt.tight_layout(); plt.savefig("v21d_results.png", dpi=150, bbox_inches="tight"); plt.close(fig)
