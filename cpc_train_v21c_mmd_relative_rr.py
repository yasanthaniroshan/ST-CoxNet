"""
v21c — MMD patient-invariance regularization + relative RR.

Excellent-only patients, patient-level 0.85/0.15 split.
RBF-kernel MMD penalty pushes embedding distributions of different
...
class TTEPredictor(nn.Module):
    ...
    def forward_with_h(self, rr_windows):
    ...
    def forward(self, rr_windows):
        return self.forward_with_h(rr_windows)[0]
...
def rbf_mmd2(x, y, bandwidths=(0.1, 1.0, 10.0)):
    """Unbiased RBF-kernel MMD^2 between two sets of embeddings."""
    ...
    for bw in bandwidths:
        g = 1.0 / (2.0 * bw ** 2)
        mmd = mmd + torch.exp(-g * xx).mean() + torch.exp(-g * yy).mean() - 2.0 * torch.exp(-g * xy).mean()
    return mmd


def batch_mmd(h, pids, bandwidths=(0.1, 1.0, 10.0)):
    """Pairwise MMD across patient groups present in one batch."""
    ...
    for i in range(len(groups)):
        for j in range(i + 1, len(groups)):
            total = total + rbf_mmd2(groups[i], groups[j], bandwidths)
            count += 1
...
try:
    ...
    config = {
        ...
        "relative_rr": True,
        "lambda_mmd_max": 0.5, "lambda_ramp_epochs": 30,
    }

    run = wandb.init(entity="eml-labs", project="CPC-New-Temporal-Ranking",
                     name="v21c-mmd-relative-rr", config=config)
    ...
    model = TTEPredictor(config["latent_dim"], config["hidden_dim"], config["dropout"]).to(device)
    ...
    pbar = tqdm(total=config["epochs"], desc="v21c MMD+relRR")
    best_val_mae = float("inf"); patience_ctr = 0

    for epoch in range(config["epochs"]):
        lam = config["lambda_mmd_max"] * min(1.0, epoch / max(config["lambda_ramp_epochs"], 1))
        model.train()
        t_loss_tte, t_loss_mmd, t_n = 0.0, 0.0, 0
        t_preds, t_actual, t_pids_ep = [], [], []

        for rr, tte, pid in train_loader:
            rr, tte, pid_dev = rr.to(device), tte.to(device), pid.to(device)
            optimizer.zero_grad()
            tte_pred, h = model.forward_with_h(rr)
            l_tte = tte_loss_fn(tte_pred, tte)
            l_mmd = batch_mmd(h, pid_dev)
            loss = l_tte + lam * l_mmd
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
            bsz = rr.size(0)
            t_loss_tte += l_tte.item() * bsz; t_loss_mmd += l_mmd.item() * bsz; t_n += bsz
        ...
        if v_mae < best_val_mae: best_val_mae = v_mae; patience_ctr = 0; torch.save(model.state_dict(), "v21c_best.pth")
        else: patience_ctr += 1

        run.log({"epoch": epoch+1, "lambda_mmd": lam,
                 "train/tte_loss": t_loss_tte/max(t_n,1), "train/mmd_loss": t_loss_mmd/max(t_n,1),
                 "train/mae": t_mae, "train/rho": t_rho,
                 "val/mae": v_mae, "val/rho": v_rho, "val/r2": v_r2, "val/pp_rho": v_mean_pp})

        if (epoch+1) % 10 == 0 or epoch == config["epochs"]-1:
            pbar.write(f"Ep {epoch+1:>3} | MAE {t_mae:.2f}/{v_mae:.2f} | ρ {t_rho:.3f}/{v_rho:.3f} | "
                       f"mmd {t_loss_mmd/max(t_n,1):.4f} | λ {lam:.2f}")
        pbar.update(1)
    ...
    model.load_state_dict(torch.load("v21c_best.pth", weights_only=True)); model.eval()
    ...
    logger.info(f"\n{'='*65}")
    logger.info(f"FINAL v21c | MAE={g_mae:.2f} | ρ={g_rho:.4f} | R²={g_r2:.4f} | pp_ρ={g_pp:.4f}")
    logger.info(f"{'='*65}")
    ...
    fig.suptitle(f"v21c MMD+relRR | MAE={g_mae:.2f} | ρ={g_rho:.3f} | pp_ρ={g_pp:.3f} | R²={g_r2:.3f}", fontsize=13)
    ...
    plt.tight_layout(); plt.savefig("v21c_results.png", dpi=150, bbox_inches="tight"); plt.close(fig)
