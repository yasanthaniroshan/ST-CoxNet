...
import random
import wandb
from Utils.Dataset.IRIDAHRVDataset import HRVSequenceDataset
from Utils.Dataset.CoxDataset.HRVSequenceCoxDataset import HRVSequenceCoxDataset
from Model.DeepSurv import DeepSurvCox
from Model.Encoder.HRVEncoder import Encoder
...
from Utils.Visualizer.AsyncVisualizer import AsyncVisualizer
...
class CPCPreModel(nn.Module):
    def __init__(self, num_targets:int,latent_dim:int, context_dim:int,number_of_heads:int):
        super().__init__()
        self.encoder = Encoder(input_dim=6, latent_dim=latent_dim)
...
def validation_step(model, dataloader, device, epoch, total_epochs):
    model.eval()
    val_loss = 0.0
    val_loss_1 = 0.0
    val_loss_2 = 0.0
    ...
    with torch.no_grad():
        for rr_windows, hrv_targets in dataloader:
            ...
            # Compute loss
            loss,loss_1,loss_2,loss_4 = training_step(model, rr_windows, hrv_targets,epoch,total_epochs)
            val_loss += loss.item()
...
@hydra.main(config_path="conf", config_name="config_irida_hrv.yaml",version_base=None)
def main(cfg: DictConfig):
    ...
    CPC_LR = 5e-4
    COX_LR = 1e-3
    CPC_EPOCHS = cfg.cpc.num_epochs
    COX_EPOCHS = cfg.cox.num_epochs
    LATENT_SIZE = 32
    CONTEXT_SIZE = 128
    ...
    CPC_BATCH_SIZE = cfg.cpc.batch_size
    COX_BATCH_SIZE = cfg.cox.batch_size
    ...
    FEATURES_LIST = ['RMSSD', 'pNN50', 'SDNN', 'alpha_1', 'sample_entropy', 'approximate_entropy']
    ...
    run = wandb.init(
        ...
        project="Prediction-PAF-Onset-with-ST-CoxNet-Using-HRV-Features",
    ...
    train_dataset = HRVSequenceDataset(dataset_path,'200x20_extracted_rr_intervals.csv',train_patients, WINDOW_SIZE, STRIDE, HORIZONS, SEQUENCE_LENGTH,features_list=FEATURES_LIST)
    hrv_scaler = train_dataset.get_scalers()
    val_dataset = HRVSequenceDataset(dataset_path,'200x20_extracted_rr_intervals.csv',val_patients, WINDOW_SIZE, STRIDE, HORIZONS, SEQUENCE_LENGTH,features_list=FEATURES_LIST, hrv_scaler=hrv_scaler)
    logger.info(f"Train samples: {len(train_dataset)}, Validation samples: {len(val_dataset)}")

    train_loader = DataLoader(
        train_dataset, batch_size=CPC_BATCH_SIZE, shuffle=True, drop_last=True,num_workers=8,pin_memory=True,persistent_workers=True,prefetch_factor=4
    )

    val_loader = DataLoader(
        val_dataset, batch_size=COX_BATCH_SIZE, shuffle=False, drop_last=False,num_workers=8,pin_memory=True,persistent_workers=True,prefetch_factor=4
    )
    ...
    for epoch in tqdm(range(1, CPC_EPOCHS + 1), desc=f"Epochs", unit="epoch"):
        ...
        val_loss,val_loss_1,val_loss_2,val_loss_4 = validation_step(model, val_loader, device,epoch,total_epochs=CPC_EPOCHS)
        run.log({
            "CPC_epoch": epoch,
    ...
    model.eval()
    model_path = os.path.join(EXPORTPATH, f'cpc_pre_model_epoch_{CPC_EPOCHS}.pth')
    torch.save(model.state_dict(), model_path)

    artifact = wandb.Artifact(name=f'cpc_pre_model', type='model')
    # artifact.add_dir(EXPORTPATH, name=f'cpc_pre_model_files')
    artifact.add_file(local_path=model_path, name=f'cpc_pre_model.pth')
    run.log_artifact(artifact)


    train_cox_dataset = HRVSequenceCoxDataset(train_patients, dataset_path, '200x20_extracted_rr_intervals.csv', limit_time_to_event=0.5, seq_len=SEQUENCE_LENGTH, hrv_scaler=hrv_scaler, features_list=FEATURES_LIST)
    validation_cox_dataset = HRVSequenceCoxDataset(val_patients, dataset_path, '200x20_extracted_rr_intervals.csv', limit_time_to_event=0.5, seq_len=SEQUENCE_LENGTH, hrv_scaler=hrv_scaler, features_list=FEATURES_LIST)
    logger.info(f"Cox Train samples: {len(train_cox_dataset)}, Cox Validation samples: {len(validation_cox_dataset)}")

    train_cox_loader = DataLoader(train_cox_dataset, batch_size=COX_BATCH_SIZE, shuffle=True, drop_last=True,num_workers=8,pin_memory=True,persistent_workers=True,prefetch_factor=4)
    validation_cox_loader = DataLoader(validation_cox_dataset, batch_size=COX_BATCH_SIZE, shuffle=False, drop_last=True,num_workers=8,pin_memory=True,persistent_workers=True,prefetch_factor=4)
    ...
    # Start visualizer
    visualizer = AsyncVisualizer(wandb_run=run)
    visualizer.start()
