"""
v16 — Multi-patient direct TTE regression.

Scales up v15 (single-patient sanity check) to ALL patients:
  - Same architecture: Encoder → GRU → MLP → TTE (minutes)
  - SR-only segments (TTE only meaningful before AF onset)
  - Patient-level train/val split (baked into CreateTemporalDataset)
  - Huber loss, AdamW + CosineAnnealing, early stopping
  - Tracks both global and per-patient Spearman ρ
"""
...
try:
    ...
    config = {
        "afib_length": 60,           # seconds
        "sr_length": int(6 * 60 * 60),
    ...
    run = wandb.init(
        ...
        name="v19-multi-patient-tte", config=config,
    )
    ...
    # ══════════════════════════════════════════════════════════════════
    #  Load data — all patients, SR-only
    # ══════════════════════════════════════════════════════════════════
    ...
    train_pids = train_ds.patient_ids.numpy()
    val_pids = val_ds.patient_ids.numpy()
    train_tte_min = train_ds.times.float().clamp(min=0) / 60.0
    val_tte_min = val_ds.times.float().clamp(min=0) / 60.0

    n_train_patients = len(np.unique(train_pids))
    n_val_patients = len(np.unique(val_pids))
    max_tte_min = float(train_tte_min.max())

    logger.info(
        f"Train: {len(train_ds)} SR segments from {n_train_patients} patients | "
        f"Val: {len(val_ds)} SR segments from {n_val_patients} patients"
    )
    logger.info(f"TTE range: 0 — {max_tte_min:.1f} min")

    train_loader = DataLoader(
        TensorDataset(train_ds.data, train_tte_min, train_ds.patient_ids),
        batch_size=config["batch_size"], shuffle=True,
        num_workers=0, pin_memory=True,
    )
    val_loader = DataLoader(
        TensorDataset(val_ds.data, val_tte_min, val_ds.patient_ids),
    ...
    for epoch in range(config["epochs"]):
        ...
        if v_mae < best_val_mae:
            best_val_mae = v_mae
            patience_ctr = 0
            torch.save(model.state_dict(), "v16_best.pth")
        else:
            patience_ctr += 1
    ...
    # ══════════════════════════════════════════════════════════════════
    model.load_state_dict(torch.load("v16_best.pth", weights_only=True))
    model.eval()
    ...
    logger.info(f"\n{'='*65}")
    logger.info("FINAL — best model on validation set (unseen patients)")
    ...
    for pid, rho in sorted(pp_rhos.items(), key=lambda x: x[1]):
        n = int((v_pids_all == pid).sum())
        logger.info(f"    Patient {pid:>3}: ρ = {rho:+.3f}  ({n} segments)")
    logger.info(f"{'='*65}")

    # ══════════════════════════════════════════════════════════════════
    #  Diagnostic plots (5-panel)
    # ══════════════════════════════════════════════════════════════════
    ...
    fig.suptitle(
        f"v16 Multi-Patient TTE  |  MAE={global_mae:.2f} min  |  "
    ...
    plt.tight_layout()
    plt.savefig("v16_multi_patient_results.png", dpi=150, bbox_inches="tight")
    run.log({
    ...
    logger.info("Plot saved to v16_multi_patient_results.png")
