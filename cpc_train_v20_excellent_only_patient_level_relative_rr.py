"""
v20 — Excellent-only TTE regression with patient-level split
and "relative RR" normalization (remove per-patient RR baseline).

What changes vs v17:
1) Patient-level split:
   - All segments from a given patient go either to train or to val.
   - No within-patient leakage by construction.

2) Relative RR (remove patient baseline):
   - For each patient p, compute mu_p and sd_p over all RR values
     belonging to that patient (across segments × steps × window positions).
   - Transform each RR value x -> (x - mu_p) / (sd_p + eps).
   - This makes the model see RR deviations relative to that patient.
"""
...
def relative_rr_remove_patient_baseline(all_data, all_pids, eps=1e-8):
    ...
    For each patient id p:
      mu_p = mean(all_data[p])
      sd_p = std(all_data[p]) (population std, unbiased=False)
      out[p] = (x - mu_p) / (sd_p + eps)
    """
    out = all_data.clone()
    ...
    for pid in pids_np:
        ...
        mu_list.append(mu)
        sd_list.append(sd)
        nseg_list.append(int(mask.sum().item()))

    stats = {
        "n_patients": int(len(pids_np)),
        "mean_mu": float(np.mean(mu_list)) if mu_list else 0.0,
        "std_mu": float(np.std(mu_list)) if mu_list else 0.0,
        "mean_sd": float(np.mean(sd_list)) if sd_list else 0.0,
        "median_sd": float(np.median(sd_list)) if sd_list else 0.0,
        "min_sd": float(np.min(sd_list)) if sd_list else 0.0,
        "max_sd": float(np.max(sd_list)) if sd_list else 0.0,
        "mean_segments_per_patient": float(np.mean(nseg_list)) if nseg_list else 0.0,
    }
    return out, stats
...
try:
    ...
    config = {
        ...
        "stride": 100,
        "window_size": 100,
        "validation_split": 0.15,  # fraction of patients into val
        # Training
        "epochs": 200,
        "batch_size": 64,
        "lr": 1e-3,
    ...
    run = wandb.init(
        ...
        name="v20-excellent-patient-level-relative-rr",
        config=config,
    )
    ...
    logger.info(
        ...
        f"{quality['summary']['n_patients']} (tier=excellent)"
    )
    ...
    # then do a true patient-level split.
    ...
    all_pids_np = all_pids.cpu().numpy()
    mask_excellent = np.isin(all_pids_np, list(excellent_pids))
    all_data = all_data[mask_excellent]
    all_times = all_times[mask_excellent]
    all_pids = all_pids[mask_excellent]

    all_tte_min = all_times.float().clamp(min=0) / 60.0

    # ── Relative RR normalization (remove per-patient RR baseline) ──
    logger.info("Applying relative RR: subtract per-patient baseline...")
    all_data_rel, rel_stats = relative_rr_remove_patient_baseline(
        all_data, all_pids, eps=config["relative_rr_eps"]
    )
    logger.info(
        "Relative RR stats: "
        f"n_patients={rel_stats['n_patients']} | "
        f"mean_mu={rel_stats['mean_mu']:.4f} | "
        f"mean_sd={rel_stats['mean_sd']:.4f} | "
        f"median_sd={rel_stats['median_sd']:.4f} | "
        f"min_sd={rel_stats['min_sd']:.4f} | "
        f"max_sd={rel_stats['max_sd']:.4f}"
    )

    # ── Patient-level split inside excellent pool ──
    unique_pids = np.unique(all_pids.cpu().numpy())
    np.random.shuffle(unique_pids)
    split_pat = int(len(unique_pids) * (1 - config["validation_split"]))
    train_pids = set(unique_pids[:split_pat].tolist())
    val_pids = set(unique_pids[split_pat:].tolist())

    # Masks over segments
    all_pids_np2 = all_pids.cpu().numpy()
    train_mask = np.isin(all_pids_np2, list(train_pids))
    val_mask = np.isin(all_pids_np2, list(val_pids))

    assert len(train_pids.intersection(val_pids)) == 0, "Patient leakage detected in split!"

    train_data = all_data_rel[train_mask]
    train_tte_min = all_tte_min[train_mask]
    train_pids_seg = all_pids[train_mask]

    val_data = all_data_rel[val_mask]
    val_tte_min = all_tte_min[val_mask]
    val_pids_seg = all_pids[val_mask]

    n_train_patients = len(np.unique(train_pids_seg.cpu().numpy()))
    n_val_patients = len(np.unique(val_pids_seg.cpu().numpy()))
    logger.info(
        f"Excellent-only PATIENT split (no leakage): "
        f"{n_train_patients} train patients | {n_val_patients} val patients"
    )
    logger.info(
        f"Train segments: {len(train_data)} | Val segments: {len(val_data)} | "
        f"TTE range (excellent only): 0 — {float(all_tte_min.max()):.1f} min"
    )

    train_loader = DataLoader(
        TensorDataset(train_data, train_tte_min, train_pids_seg),
    ...
    val_loader = DataLoader(
        TensorDataset(val_data, val_tte_min, val_pids_seg),
        batch_size=config["batch_size"],
        shuffle=False,
        num_workers=4,
        pin_memory=True,
    ...
    # ── Training loop ──
    pbar = tqdm(total=config["epochs"], desc="Training (v20 patient-level)")
    ...
    for epoch in range(config["epochs"]):
        ...
        for rr, tte, pid in train_loader:
            ...
            t_actual.append(tte.detach().cpu().numpy())
        ...
        with torch.no_grad():
            for rr, tte, pid in val_loader:
                ...
                v_n += rr.size(0)
                v_preds.append(pred.detach().cpu().numpy())
                v_actual.append(tte.detach().cpu().numpy())
        ...
        if v_mae < best_val_mae:
            best_val_mae = v_mae
            patience_ctr = 0
            torch.save(model.state_dict(), "v20_best.pth")
        else:
            patience_ctr += 1
    ...
    model.load_state_dict(torch.load("v20_best.pth", weights_only=True))
    model.eval()
    ...
    logger.info(f"\n{'='*65}")
    logger.info("FINAL — v20 best model on patient-level val (excellent only)")
    ...
    # Diagnostic plots (similar layout to v17)
    ...
    fig.suptitle(
        f"v20 Excellent-Only Patient-Level Relative RR | "
        f"MAE={global_mae:.2f} min | ρ_global={global_rho:.3f} | "
        f"ρ_patient={mean_pp_rho:.3f} | R²={global_r2:.3f} | "
        f"{len(pp_rhos)} val patients",
        fontsize=14,
    )
    ...
    plt.tight_layout()
    out_png = "v20_excellent_only_patient_level_relative_rr_results.png"
    ...
    run.log(
        {
            ...
            "final/relative_rr_summary": rel_stats,
        }
    )
