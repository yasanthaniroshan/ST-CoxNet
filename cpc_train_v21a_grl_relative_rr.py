"""
v21a — GRL adversarial patient-invariance + relative RR.

Excellent-only patients, patient-level 0.85/0.15 split.
Gradient Reversal Layer forces encoder/GRU to produce embeddings
that are useful for TTE prediction but adversarial to patient-ID
classification.
"""
...
try:
    ...
    run = wandb.init(entity="eml-labs", project="CPC-New-Temporal-Ranking",
                     name="v21a-grl-relative-rr", config=config)
    ...
    if config["relative_rr"]:
        ...
        all_data = relative_rr_remove_patient_baseline(all_data, all_pids)
    ...
    # Patient-ID class mapping (train patients only)
    ...
    pbar = tqdm(total=config["epochs"], desc="v21a GRL+relRR")
    ...
    for epoch in range(config["epochs"]):
        ...
        if v_mae < best_val_mae:
            best_val_mae = v_mae; patience_ctr = 0
            torch.save(model.state_dict(), "v21a_best.pth")
        else:
            patience_ctr += 1
        ...
        if (epoch+1) % 10 == 0 or epoch == config["epochs"]-1:
            pbar.write(f"Ep {epoch+1:>3} | MAE {t_mae:.2f}/{v_mae:.2f} | "
                       f"ρ {t_rho:.3f}/{v_rho:.3f} | pat_acc {pat_acc:.3f} | λ {lam:.2f}")
        pbar.update(1)
        if patience_ctr >= config["patience"]:
            pbar.write(f"Early stop ep {epoch+1} | best MAE {best_val_mae:.2f}")
            break

    pbar.close()

    # ── Final evaluation ──
    model.load_state_dict(torch.load("v21a_best.pth", weights_only=True)); model.eval()
    ...
    logger.info(f"\n{'='*65}")
    logger.info(f"FINAL v21a | MAE={g_mae:.2f} | ρ={g_rho:.4f} | R²={g_r2:.4f} | pp_ρ={g_pp:.4f}")
    logger.info(f"{'='*65}")

    # ── Centroid diagnostic ──
    # Need a 3-tuple val_loader for centroid_diagnostic
    ...
    fig.suptitle(f"v21a GRL+relRR | MAE={g_mae:.2f} | ρ={g_rho:.3f} | pp_ρ={g_pp:.3f} | R²={g_r2:.3f}", fontsize=13)

    ax = fig.add_subplot(1, 3, 1)
    lim = max(v_actual.max(), v_preds.max()) * 1.05
    ax.scatter(v_actual, v_preds, alpha=0.4, s=10); ax.plot([0,lim],[0,lim],"r--",lw=1)
    ...
    plt.tight_layout()
    plt.savefig("v21a_results.png", dpi=150, bbox_inches="tight"); plt.close(fig)
