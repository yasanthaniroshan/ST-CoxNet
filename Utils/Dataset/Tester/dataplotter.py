...
Segment_Name = unique_segments[0]  # Use first segment, or change to specific segment
...
# Get the last 3000 sample indices worth of data
max_end_idx = segment_data['end_idx'].max()
min_start_idx_for_last_3000 = max_end_idx - 3000

print(f"Filtering for last 3000 indices: {min_start_idx_for_last_3000} to {max_end_idx}")

# Filter for last 3000 sample indices
filtered_data = segment_data[
    segment_data['end_idx'] >= min_start_idx_for_last_3000
].copy()

print(f"Found {len(filtered_data)} windows in last 3000 indices")
...
if len(hrv_rmssd_values) > 0:
    ...
    plt.title(f'HRV RMSSD - Last 3000 Indices for {Segment_Name}')
    ...
    plot_filename_base = f"{Segment_Name}_RMSSD_last_3000_indices"
    ...
    # Add statistics
    print(f"\nHRV_RMSSD Statistics for {Segment_Name} (Last 3000 indices):")
